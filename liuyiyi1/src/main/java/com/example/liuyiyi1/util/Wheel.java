...
public class Wheel {
    ...
    public Wheel(GpioController gpioController, Pin first, Pin second, Pin pwn) throws InterruptedException {
        this(gpioController, first, second);
        //设置pwm输出
        Wheel.pwn = gpioController.provisionPwmOutputPin(pwn);
        com.pi4j.wiringpi.Gpio.pwmSetMode(com.pi4j.wiringpi.Gpio.PWM_MODE_MS);
        com.pi4j.wiringpi.Gpio.pwmSetRange(range);
        com.pi4j.wiringpi.Gpio.pwmSetClock(divisor);
        Wheel.pwn.setPwm(0);
        Wheel.pwn.setShutdownOptions(true, PinState.LOW, PinPullResistance.OFF);
        while(true){

            int range = getRange();
            System.out.print("获取pwm的输入范围:"+range);
            setPwnRate(500);
            Thread.sleep(1000);
             range = getRange();
            System.out.print("获取pwm的输入范围:"+range);
